...
def get_all_hashtags_and_links(tweet=TWEET):
    """Get all hashtags and links from the tweet text
       ...
       Return this list.
    """
    hashtags_and_links = re.findall(
        "#\w+|http[s]?://(?:[a-zA-Z]|[0-9]|[$-_@.&+#]|[!*\(\),]|(?:%[0-9a-fA-F][0-9a-fA-F]))+",
        tweet,
    )
...
def match_first_paragraph(html=HTML):
    ...
    return first_paragraph.groups()[0]
...
def main():
    ...
    hashtags_and_links = get_all_hashtags_and_links(tweet)
    first_paragraph = match_first_paragraph()
    pass
